...
const Clip_1 = require("./Clip/Clip");
class User {
    ...
    async getClips() {
        try {
            ...
            return data.items.map((item) => {
                return {
                    getAllTypes: () => {
                        return item.media.video_versions.map(version => new Clip_1.Clip(version));
                    },
                    first: new Clip_1.Clip(item.media.video_versions[0])
                };
            });
        }
        catch (error) {
